...
function init_darts_game_sc () {
	...
	sc.update = function () {
		...
		switch (sc.step) {
			...
			case 1: // choix de la length
				...
				if (sc.can_click && window.all.mouse.is_down == true) {
					...
					sc.dart.end_y = sc.dart.start_y - Math.sin(sc.vector.angle + window.all.half_PI) * sc.vector.len;
				}
			break;
			case 2: // animation
			...
			case 3: // score
				...
				var d = Math.sqrt(dist_2d_sqrt(sc.target, sc.dart));
				var diff = d / Math.sqrt(dist_2d_sqrt(sc.target, { x: sc.dart.start_x, y: sc.dart.start_y }));

				sc.scores[sc.round_nb] = 100 - (diff * 100 | 0);
		...
		if (sc.scores[sc.round_nb]) {
			window.all.ctx.fillStyle = "#fe2";
			window.all.ctx.font = "48px Impact";
			window.all.ctx.fillText(sc.scores[sc.round_nb], window.all.width >> 1, window.all.height >> 1);
			window.all.ctx.fillStyle = "#f80";
			window.all.ctx.font = "32px Impact";
			window.all.ctx.fillText("Highscore: " + sc.high_score, window.all.width >> 1, window.all.height * 0.75);
		}
